...
export const ErrorResponses = {
  ...
  unauthorized: (message: string = 'No autorizado') => errorResponse(message, 401, 'UNAUTHORIZED'),

  forbidden: (message: string = 'Acceso denegado') => errorResponse(message, 403, 'FORBIDDEN'),

  conflict: (message: string = 'Conflicto') => errorResponse(message, 409, 'CONFLICT'),
